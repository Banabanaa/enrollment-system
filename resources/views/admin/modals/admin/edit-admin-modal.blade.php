...
<div id="editAdminModal-{{ $admin->id }}" class="modal-overlay hidden scrollbar-thin scrollbar-thumb-green-500 scrollbar-track-gray-100">
    <div class="modal-container">
        ...
        <div class="modal-header border-b border-border-color mb-6 flex justify-between items-center">
            ...
            <button type="button" class="modal-close text-gray-600 hover:text-gray-900" onclick="toggleModal('editAdminModal-{{ $admin->id }}')">
        ...
        <div class="max-h-96 overflow-y-auto">
            <form action="{{ route('admin.manage.admin.update', $admin->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-6 pr-5 pl-3">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label for="last_name" class="form-label text-sm text-dark">Full Name</label>
                            <input type="text" class="form-control w-full px-3 py-2 bg-input rounded-lg text-sm border-0 focus:outline-none focus:ring-2 focus:ring-primary" name="name" value="{{ $admin->name }}" required>
                        </div>
                        <div>
                            <label for="contact_number" class="form-label text-sm text-dark">Contact Number</label>
                            <input type="text" class="form-control w-full px-3 py-2 bg-input rounded-lg text-sm border-0 focus:outline-none focus:ring-2 focus:ring-primary" name="contact_number" value="{{ $admin->contact_number }}">
                        </div>
                        <div>
